...
function train_svm_struct
    ...
    parm.verbose = 1 ;
    tic
    model = svm_struct_learn(' -c 1.0 -o 1 -v 1 -w 4 ', parm) ;
    training_time = toc;
    w = model.w ;
    ...
    save('train_svm_struct_3_gibbs.mat', 'acc', 'training_time', 'testing_time', 'training_samples', 'testing_samples');
end
% ------------------------------------------------------------------
...
function psi = featureCB(param, x, y)
  ...
  psi = sparse([res ;double(y(1) ~= y(2)); double(y(2) ~= y(3)) ;double(y(1) ~= y(3))]);
end
